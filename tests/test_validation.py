...
def test_get_links_handles_insufficient_tables():
    ...
    with patch('ubu.models.requests.get') as mock_get:
        ...
        mock_response.content = b"<html><body><table><tr><td>Only one table</td></tr></table></body></html>"
        mock_get.return_value = mock_response
...
def test_work_download_url_is_attribute_not_method():
    ...
    assert not hasattr(work, 'set_download_url') or not callable(getattr(work, 'set_download_url', None))
...
def test_download_summary_stats():
    ...
    from ubu.models import Page, Artist, Work
    import io
    import sys
