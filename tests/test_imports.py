...
def test_module_all_exports():
    ...
    assert len(ubu.__all__) == 20  # 7 constants + 5 models + 5 functions + 2 file_index + 1 twitter
...
def test_constants_available():
    ...
    constants = ['BASE_URL', 'FILM_URL', 'BASE_FILM_URL', 
                 'DOWNLOAD_PATH', 'ERROR_URL', 'BROKEN_PAGES']
...
def test_downloader_functions_available():
    ...
    functions = ['download_random_work_from', 'download_all_works_from',
                 'full_download_run', 'get_url_from_text', 'download_from_tweet']
